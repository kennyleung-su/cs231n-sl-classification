...
class ResnetEncodingDatasetCache(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        logging.debug('Getting gesture video for: {0}: {1}'.format(idx, video_dir)) 
    ...
    def populate_encoding_data(self, gesture_labels):
        ...
        for label in gesture_labels:
            ...
            for video_dir in video_dirs:
                ...
                full_dir = os.path.join(self._data_dir, item['video_dir'])
                data.append({
                    'label': self._labels_to_indices_dict[label],
                    'frames': self.read_frame_tensors_from_dir(full_dir, self._data_type),
                    'seq_len': seq_len
                })
